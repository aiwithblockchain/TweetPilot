...
use crate::task_database::{CreateTaskAiSessionInput, ExecutionResult, Task, TaskDatabase};
...
pub async fn execute_task_ai_session(
    ...
    let mut result = match outcome {
        ...
        QueryOutcome::Cancelled => ExecutionResult {
            id: execution_id,
            task_id: task.id.clone(),
            run_no: Some(run_no),
            session_code: Some(session_code),
        ...
        QueryOutcome::BudgetExceeded { .. } => ExecutionResult {
            id: execution_id,
            task_id: task.id.clone(),
            run_no: Some(run_no),
            session_code: Some(session_code),
        ...
        QueryOutcome::Error(error) => ExecutionResult {
            id: execution_id,
            task_id: task.id.clone(),
            run_no: Some(run_no),
            session_code: Some(session_code),
