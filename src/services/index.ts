...
import { commentInputRepository } from "../data/commentInputRepositoryInstance.js";
...
export * from "./executionEligibilityService.js";
export * from "./executionFailureHandler.js";
export * from "./executionProtectionService.js";
import { createReplyTaskCreationService } from "./replyTaskCreationService.js";
export * from "./executionPreparationService.js";
export * from "./protectedTaskExecutionOrchestrator.js";
export * from "./taskExecutionOrchestrator.js";
export * from "./taskExecutionResultWriter.js";
...
function createFallbackAIProvider(): IAIProvider {
	return {
		async generateText(
			...
			const commentMatch = prompt.match(
				/你需要回复以下评论:\n(.+?)(?:\n\n|$)/s,
			);
			...
			const variant =
				typeof options?.temperature === "number" && options.temperature >= 0.85
					...
					: typeof options?.temperature === "number" &&
						...
						: "稳健版";
			const knowledgeHint =
				knowledgeLines.length > 0 ? ` 参考信息：${knowledgeLines[0]}。` : "";
...
export const reviewQueueService = createReviewQueueService({
	replyTaskRepository,
	candidateReplyRepository,
	commentInputRepository,
});
export const taskOwnershipService = createTaskOwnershipService({
	replyTaskRepository,
});
