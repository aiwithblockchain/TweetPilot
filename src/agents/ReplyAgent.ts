...
import type {
	IKnowledgeBase,
	KnowledgeItem,
} from "../knowledge/IKnowledgeBase.js";
...
export class ReplyAgent {
	...
	assessRisk(reply: string): "low" | "medium" | "high" {
		...
		if (
			HIGH_RISK_KEYWORDS.some((keyword) => normalizedReply.includes(keyword))
		) {
			return "high";
		}

		if (
			MEDIUM_RISK_KEYWORDS.some((keyword) => normalizedReply.includes(keyword))
		) {
			return "medium";
		}
	...
	calculateConfidence(
		...
		return (
			Math.round(Math.max(0, lengthScore - riskPenalty[riskLevel]) * 100) / 100
		);
	}
