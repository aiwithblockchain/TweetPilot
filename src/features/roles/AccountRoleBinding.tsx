...
type AccountRoleBindingProps = {
  ...
  onBindRole: (accountId: string, roleId: string, isDefault: boolean) => Promise<void> | void;
  ...
  return (
    <section className="panel role-panel">
      <div className="binding-header">
        <div>
          <p className="panel-title">Account Role Binding</p>
          <p className="muted">Bind roles and mark a default role per account.</p>
        </div>
                  ...
                  <div className="action-row">
                    {defaultRoleId === role.id ? (
                      ...
                      <button
                        ...
                        disabled={disabled}
                        onClick={() => onSetDefaultRole(selectedAccount.id, role.id)}
                      >
                        Set Default
                      </button>
                    )}
              ...
              {workspaceRoles.map((role) => {
                ...
                return (
                  <div key={role.id} className="binding-item">
                    ...
                    <div className="action-row">
                      {isBound ? (
                        <span className="default-pill default-pill-muted">Bound</span>
                      ) : (
                        <>
                          <button
                            type="button"
                            className="secondary-button"
                            disabled={disabled}
                            onClick={() => onBindRole(selectedAccount.id, role.id, false)}
                          >
                            Bind
                          </button>
                          <button
                            ...
                            disabled={disabled}
                            onClick={() => onBindRole(selectedAccount.id, role.id, true)}
                          >
                            Bind As Default
                          </button>
                        </>
