 ...
 */
export function getSamplesByWorkspace(
	workspaceId: string,
): CreateCommentInputParams[] {
	return STANDARD_COMMENT_SAMPLES.filter(
		(sample) => sample.workspaceId === workspaceId,
	);
}
 ...
 */
export function getSamplesByAccount(
	accountId: string,
): CreateCommentInputParams[] {
	return STANDARD_COMMENT_SAMPLES.filter(
		(sample) => sample.accountId === accountId,
	);
}
