...
export class LocalBridgeClient {
	...
	private async fetch<T>(path: string, options: RequestInit = {}): Promise<T> {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`LocalBridge API error: ${response.status} ${response.statusText}`,
				);
			}
	...
	async getTweet(
		tweetId: string,
		tabId?: number,
	): Promise<LocalBridgeTweetDetailResponse> {
		const params = tabId ? `?tabId=${tabId}` : "";
		return this.fetch<LocalBridgeTweetDetailResponse>(
			`/api/v1/x/tweets/${tweetId}${params}`,
		);
	}
	...
	async getTweetReplies(
		...
		return this.fetch<LocalBridgeRepliesResponse>(
			`/api/v1/x/tweets/${tweetId}/replies${query}`,
		);
	}
}
