...
import type { LocalBridgeTweetResult, LocalBridgeRepliesResponse } from './types';
...
function extractTweetFromResult(result: LocalBridgeTweetResult | undefined): AdaptedReply | null {
  if (!result || !result.rest_id || !result.legacy?.full_text) {
    return null;
  }

  const authorId = result.core?.user_results?.result?.rest_id;
  const authorScreenName = result.core?.user_results?.result?.legacy?.screen_name;
