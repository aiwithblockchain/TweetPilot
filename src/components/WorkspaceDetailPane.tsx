  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center px-6">
        <div className="rounded border border-[var(--color-border)] bg-[var(--color-surface)] px-6 py-8 text-center">
          <Loader2 className="w-5 h-5 text-[#9CDCFE] animate-spin mx-auto" />
          <p className="text-sm text-[var(--color-text-secondary)] mt-3">正在加载 {item.label} ...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="p-6 space-y-5">
        ...
        <Panel icon={<FolderClosed className="w-5 h-5 text-[#D7BA7D]" />} title="目录概览">
          <div className="text-sm text-[var(--color-text)] leading-6 space-y-2">
            <p>当前选中的是文件夹节点。v1 支持在左侧展开子目录、刷新目录，以及在当前目录下新建文件或文件夹。</p>
            <p className="text-[var(--color-text-secondary)] break-all">路径：{folderSummary.path}</p>
          </div>
        </Panel>
      </div>
    )
    ...
    <div className="p-6 space-y-5">
      ...
      {fileContent.contentType === 'text' && (
        <Panel icon={<FileText className="w-5 h-5 text-[#4EC9B0]" />} title="文本预览">
          <pre className="rounded border border-[var(--color-border)] bg-[var(--color-bg)] p-4 text-xs text-[var(--color-text)] leading-6 overflow-auto whitespace-pre-wrap min-h-[260px]">
      ...
      {fileContent.contentType === 'image' && (
        <Panel icon={<ImageIcon className="w-5 h-5 text-[#9CDCFE]" />} title="图片预览">
          <div className="rounded border border-[var(--color-border)] bg-[var(--color-bg)] p-4 min-h-[280px] flex items-center justify-center overflow-auto">
            {fileContent.imageSrc ? (
              <img src={fileContent.imageSrc} alt={fileContent.name} className="max-w-full max-h-[560px] object-contain" />
            ) : (
              <div className="text-sm text-[var(--color-text-secondary)]">图片数据不可用</div>
            )}
          </div>
        </Panel>
      )}
      ...
      {fileContent.contentType === 'unsupported' && (
        <Panel icon={<FileQuestion className="w-5 h-5 text-[#D7BA7D]" />} title="暂不支持的文件类型">
          <div className="rounded border border-dashed border-[var(--color-border)] bg-[var(--color-bg)] min-h-[220px] flex flex-col items-center justify-center text-sm text-[var(--color-text-secondary)] text-center px-6">
...
function Header({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div>
      ...
      <p className="text-sm text-[var(--color-text-secondary)] mt-1 leading-6">{subtitle}</p>
    </div>
  )
}

function Panel({ icon, title, children }: { icon: React.ReactElement; title: string; children: React.ReactNode }) {
  return (
