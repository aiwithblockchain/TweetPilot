...
export function TaskActionBar({ task, onChanged }: TaskActionBarProps) {
  const [executing, setExecuting] = useState(false)
  const [result, setResult] = useState<ExecutionResult | null>(null)
  const [actionError, setActionError] = useState<string | null>(null)
  const [actionLoading, setActionLoading] = useState<'pause' | 'resume' | 'execute' | null>(null)
        ...
        <div className="flex flex-wrap gap-2">
          {task.type === 'immediate' ? (
            ...
            <button
              ...
              className="h-9 px-4 rounded border border-[#2A2A2A] text-sm text-[#CCCCCC] hover:bg-[#2A2A2A] transition-colors disabled:opacity-50"
            >
              {actionLoading === 'pause' ? '暂停中...' : '暂停任务'}
            </button>
          ) : (
            <button
              type="button"
