import type { PlatformState, Workspace, Account, Instance, ExecutionChannel } from '../domain/types';
...
class PlatformStateManager {
  ...
  getAccounts(workspaceId?: string): Account[] {
    if (workspaceId) {
      return this.state.accounts.filter((acc) => acc.workspaceId === workspaceId);
    }
  ...
  getInstances(accountId?: string): Instance[] {
    if (accountId) {
      return this.state.instances.filter((inst) => inst.accountId === accountId);
    }
