...
**重要说明**：TweetPilot V1 已经完成了 Phase 1 的开发（T1-T5 任务卡），包括：
- 平台壳层、评论输入链路
- Reply Agent 生成候选回复（基于 Claude API 直接调用）
- 任务治理、审核队列
- 执行通道、本地扩展执行

**V2 架构变化**：
- V2 采用完全不同的架构（产品中心化 + Claurst Host Manager + MCP 服务器）
- V1 代码与 V2 架构关系不大，**可以完全抛弃，不是必须保留**
- 详细的代码评估和迁移策略见 [06-技术选型与现有代码评估.md](docs/v2/06-技术选型与现有代码评估.md) 和 [07-从V1到V2的迁移策略.md](docs/v2/07-从V1到V2的迁移策略.md)
