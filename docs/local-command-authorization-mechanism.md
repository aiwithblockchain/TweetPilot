...
为了兼顾安全性与使用体验，本地工具授权不应长期停留在“每次都只弹一次”的单层模型，而应明确分层演进目标。

建议把授权层级理解为以下 4 层：

#### Level 1：请求级授权（request-level）
- 只针对当前这一次工具调用生效
- 例如：允许这次写文件、拒绝这次执行某条命令
- 这是 MVP 第一阶段默认采用的模型

#### Level 2：Session 级授权（session-level）
- 对当前 AI session 生命周期内的同类请求复用授权结果
- 适合用户正在连续协作、但不希望每一步都重复确认的场景
- session 结束后失效

#### Level 3：工作区 / 工程级授权（workspace-level）
- 对当前工作区长期记忆某类权限规则
- 适合同一项目内反复进行相似操作的场景
- 需要支持查看、撤销与范围说明

#### Level 4：全局默认策略（global-level）
- 跨工作区生效的全局安全偏好或系统底线
- 更适合承载：
  - 明确禁止的极高风险操作
  - 低风险只读操作的默认策略
- 不建议用于“全局允许任意写入/删除”这类高风险放权

需要特别说明的是：

> **MVP 第一阶段只实现请求级授权，不实现 session/workspace/global 的授权记忆。**

也就是说，第一阶段里的“允许一次 / 拒绝”都只作用于当前请求，不会自动推广到当前 session、当前工程或所有工程。
...
为了避免授权语义含糊，必须明确“拒绝一次”在系统中的真实含义。
...
- 用户本次点了拒绝，不代表系统会自动记住并拒绝后续相似请求
- 如果 AI 后续再次发起类似请求，系统仍然会重新进入：
  - 风险判断
  - `allow / ask / deny` 决策链路

也就是说，**第一阶段的拒绝是 request-level deny，而不是 session-level deny 或 workspace-level deny。**

### 为什么第一阶段不做自动记忆拒绝

原因有两个：

1. 先把最小授权闭环做真，避免一开始把规则记忆做复杂
2. 拒绝记忆要先定义“什么叫同类请求”，否则很容易产生误杀或规则失控

例如，下面几种请求并不一定应该被视为完全相同：

- `file.write + workspace`
- `file.write + external`
- `command.write + workspace`
- `command.destructive + external`

如果没有稳定的归一化维度，就不适合在第一阶段引入自动拒绝记忆。

### 后续建议演进方向

后续可以逐步补齐更接近 IDE 插件体验的授权层级，例如：
...
- 当前阶段先把 request-level 授权闭环做真
- session-level / workspace-level 的允许与拒绝记忆放到后续阶段实现
- 后续如要扩展，必须建立“同类请求”归一化规则，而不是简单按字符串匹配
